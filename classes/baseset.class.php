<?php
	class BaseSet{
		...
		static function getSet(){
			...
			global $styleList,$pictureSize,$thumbSize,$homeid;
		...
		static function writeConfig($post){
			...
			$rep=array(	
					...
					"homeid = array('first' => {$post['first']}, 'second' => {$post['second']}, 'three' => {$post['three']}, 'four' => {$post['four']}, 'five' => {$post['five']}, 'six' => {$post['six']});"
				);
