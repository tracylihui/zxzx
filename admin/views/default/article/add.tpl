...
<div id="main">
	<div class="head-dark-box">
		<div class="tit">内容管理->文章管理->添加文章</div>
	</div>
	...
	<form name="article" method="post" action="<{$url}>/insert" enctype="multipart/form-data">
		<div class="msg-box">
			<ul class="viewmess">
				...
				<li class="dark-row"> <span class="col_width">文章类别&nbsp;</span><input type="hidden"  name="pid" value="<{$select.id}>" /> <{ $select.title}>
				<li class="dark-row"> <span class="col_width">发布日期<span class="red_font">*</span></span> <{$posttime}> &nbsp; </li>
				<li class="dark-row"><span class="col_width">同时发布到&nbsp;</span> <input type="radio"  name="news" <{if $post.news eq 0}>checked<{/if}> value="0">不同步 <input type="radio"  name="news" <{if $post.news eq 1}>checked<{/if}>  value="1">新闻 <input type="radio"  name="news" <{if $post.news eq 2}>checked<{/if}> value="2">公告 </li>
				<li class="dark-row"><span class="col_width">是否直接链接到目标页面</span> <input type="radio"  name="ishttp" <{if $post.ishttp eq 0}>checked<{/if}> value="0">否 <input type="radio"  name="ishttp" <{if $post.ishttp eq 1}>checked<{/if}>  value="1">是
					&nbsp;&nbsp;&nbsp;&nbsp;如果选择是，请在文章内容中添加超链接 </li>
